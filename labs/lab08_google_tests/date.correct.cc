...
std::string Date::GetDate() const {
  ...
  std::string date = "";
  snprintf(numstr, sizeof(numstr), "%d", yyyy_);
  date = date + numstr + "-";
  if (mm_ < 10)
    date = date + "0";
  snprintf(numstr, sizeof(numstr), "%d", mm_);
  date = date + numstr + "-";
  if (dd_ < 10)
    date = date + "0";
  snprintf(numstr, sizeof(numstr), "%d", dd_);
  date = date + numstr;
  return date;
}
...
void Date::PrintDate(bool newline) const {
  std::cout << yyyy_ << "-" << std::setw(2) << std::setfill('0') << mm_ << "-"
...
void Date::PrintUsDate(bool newline) const {
  std::cout << std::setw(2) << std::setfill('0') << mm_ << "-" << std::setw(2)
  << std::setfill('0') << dd_ << "-" << yyyy_;
  if (newline)
    std::cout << std::endl;
}
