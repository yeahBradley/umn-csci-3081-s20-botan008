/**
 * @file r_my_passenger_generator.cc
 *
...
rMyPassengerGenerator::rMyPassengerGenerator(std::list<double> probs,
...
int rMyPassengerGenerator::GeneratePassengers() {
  ...
  int count = 0;
  for (stop_iter = stops_.begin(); (*stop_iter)->GetId()
                         < last_stop_index; stop_iter++) {
      ...
      // This generates (stop.id_ + 1) passengers
      for (int i = 0; i <= (*stop_iter)->GetId(); i++) {
          Passenger * tmp = PassengerFactory::
                              Generate(stop_index,
                                     last_stop_index);
          passengers_added += (*stop_iter)->AddPassengers(tmp);
